...
def split_lean_theorem(lean_code):
    ...
    match = re.search(r'^(theorem|lemma)\s+[\s\S]+?:= by', lean_code, re.DOTALL | re.MULTILINE)
    if match:
        # 提取声明部分
...
def extr_fls_flp(intext):
    NLS = re.search(r'/-(.*?)-/', intext, re.DOTALL).group(1).strip()
    
    if ":= by" in intext:
        try:
            FLS, FLP = intext.rsplit(':= by', 1)   #split_lean_theorem(item['output'])        
        except:
            print(intext)
            raise
            #print(fls_in)
...
def convert_format(org_data, task, dataset):
    ...
    for item in tqdm(org_data):
        ...
        NLS, FLS, FLP = extr_fls_flp(item['full_proof'])
       
        idx = get_md5(item['full_proof'])
...
def main():
    ...
    indir = '../org/lean_workbook_proofs'
    outdir = '../processed/'
    task = 'FLPG'
