...
output_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-3/1_pretrain/MUSTARDSauce/MUSTARDSauce-NLS-NLP-FLP.json'
...
def main():
    ins_path = '/data3/ftpdata/kmwang5/Lean-dataset/instructions/lean3-NLS\FLS-NLP-FLP.json'
    ...
    for data in tqdm(indata):
        ...
        if instruction['position'] == 'head':
            context = add_ret(instruction['instruction'] + '\n' + inputtext) + add_ret(data['informal_proof'] + '\n' + data['formal_proof'])
        else:
            context = add_ret(inputtext + '\n' + instruction['instruction']) + add_ret(data['informal_proof'] + '\n' + data['formal_proof'])
        idx = get_md5(context)
