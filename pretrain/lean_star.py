...
def main():
    ins_path = '../../instructions/lean_star_cot_instruction.json'
    ...
    for data in tqdm(indata):
        ...
        inputtext = data['input'].replace(org_ins, instruction['instruction'])
        inputtext = inputtext.replace('<|im_start|>user', '').replace('<|im_start|>assistant', '').replace('<|im_end|>', '')
        data['output'] = data['output'].replace('<|im_end|>', '')
        context = add_ret(inputtext) + add_ret(data['output'])
        idx = get_md5(context)
