...
output_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-4/1_pretrain/lean-nl/lean-nl-FLS-FLP.json'
datasource = 'lean-nl'
...
def main():
    ...
    for data in tqdm(indata):
        ...
        inputtext = data['formal_statement'].replace(org_ins, '')
        if instruction['position'] == 'head':
            context = add_ret(instruction['instruction'] + '\n' + inputtext) + add_ret(data['full_proof'].split('= by\n  ')[-1])
        else:
            context = add_ret(inputtext + '\n' + instruction['instruction']) + add_ret(data['full_proof'].split('= by\n  ')[-1])
        idx = get_md5(context)
