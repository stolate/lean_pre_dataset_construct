...
output_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-4/1_pretrain/lean-nl/lean-nl-FLS-NLP-FLP.json'
datasource = 'lean-nl'
...
def main():
    ...
    for data in tqdm(indata):
        ...
        inputtext = data['natural_language_statement']

        if instruction['position'] == 'head':
            context = add_ret(instruction['instruction'] + '\n' + inputtext) + add_ret(data['nl_solution'] + '\n' + data['full_proof'])
        else:
            context = add_ret(inputtext + '\n' + instruction['instruction']) + add_ret(data['nl_solution'] + '\n' + data['full_proof'])
        idx = get_md5(context)
