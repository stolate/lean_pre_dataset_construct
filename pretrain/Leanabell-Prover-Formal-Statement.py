...
input_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-4/0_ori/Leanabell-Prover-Formal-Statement/formal_statement.jsonl'
output_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-4/1_pretrain/Leanabell-Prover-Formal-Statement/Leanabell-Prover-Formal-Statement.json'
back_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-4/1_pretrain/Leanabell-Prover-Formal-Statement/backtrans_Leanabell-Prover-Formal-Statement.json'
datasource = 'Leanabell-Prover-Formal-Statement'
...
def main():
    ...
    for data in tqdm(indata):
        #print(data)
        NLS = data['problem'].strip()
        instruction = random.choice(instructions)
        if re.search(':=$', data['formal_statement'].strip()):
            data['formal_statement'] = data['formal_statement'].strip() + ' by'
        elif re.search('sorry$', data['formal_statement'].strip()):
            data['formal_statement'] = re.sub('sorry$', '', data['formal_statement']).strip()

        if instruction['position'] == 'head':
            if random.random() < 0.5: 
                context = add_ret(instruction['instruction'] + '\nNatural language:\n' + NLS) + add_ret(response.format(target=data['formal_statement']))
            else:
                context = add_ret(instruction['instruction'] + NLS) + add_ret(response.format(target=data['formal_statement']))
        else:
            if random.random() < 0.5: 
                context = add_ret('Natural language:\n' + NLS + '\n' + instruction['instruction']) + add_ret(response.format(target=data['formal_statement']))
            else:
                context = add_ret(NLS + '\n' + instruction['instruction']) + add_ret(response.format(target=data['formal_statement']))
        ...
        if back_instruct['position'] == 'head':
            if random.random() < 0.5:
                back_context = add_ret(back_instruct['instruction'] + '\nLean4 code:\n' + data['formal_statement']) + add_ret(back_response.format(intarget=NLS))
            else:
                back_context = add_ret(back_instruct['instruction'] + data['formal_statement']) + add_ret(back_response.format(intarget=NLS))
        else:
            if random.random() < 0.5:
                back_context = add_ret('Lean4 code:\n' + data['formal_statement'] + '\n' + back_instruct['instruction']) + add_ret(back_response.format(intarget=NLS))
            else:
                back_context = add_ret(data['formal_statement'] + '\n' + back_instruct['instruction']) + add_ret(back_response.format(intarget=NLS))
