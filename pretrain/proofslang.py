...
input_p = '/data3/ftpdata/kmwang5/Lean-dataset/自然语言证明题/0_ori/prooflang/proofs.tsv'
output_p = '/data3/ftpdata/kmwang5/Lean-dataset/自然语言证明题/1_pretrain/prooflang/prooflang.json'
datasource = 'prooflang'
...
def main():
    indata = pd.read_csv(input_p, sep='\t', header=0)
    data_lst = []
    for _, data in tqdm(indata.iterrows(), total=len(indata)):
        try:
            inputtext = data['proof']
