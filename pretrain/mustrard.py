...
input_p = '/data6/ftpdata/yilei4/math/data/train/org/MUSTARDSauce/data/subset_filtered'    #sys.argv[1]
output_p = ''     #sys.argv[2]
datasource = 'MUSTARDSauce'   #sys.argv[3]
...
def main():
    ...
    for filename in os.listdir(input_p):
        ...
        indata = indent_json2dict(path)
        print(indata)
        text = '{}\ninformal proof:{}\n'.format(indata['informal_statement'], indata['informal_proof'])
        raise
    for data in tqdm(indata):
        instruction = random.choice(ins_lst)
        #print(data)
        context = add_ret(instruction['instruction'] + '\nNatural language: ' + data['input']) + add_ret(data['output'])
