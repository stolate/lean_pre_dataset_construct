...
input_p = '/data2/xyzhou15/数学合成数据/LEAN4/MMA-main/0_ori/data/MMAdataset/lean_train.jsonl'
output_p = '/data6/ftpdata/yilei4/math/data/train/pretrain_format/MMA.json'
back_p = '/data6/ftpdata/yilei4/math/data/train/pretrain_format/backtrans_MMA.json'
datasource = 'MMA'
...
def main():
    ...
    for data in tqdm(indata):
        #print(data)
        NLS = data['input'].lstrip("Statement in natural language:").rstrip("Translate the statement in natural language to Lean:").strip()
        instruction = random.choice(instructions)['instruction']
        context = add_ret(instruction + '\nNatural language:\n' + NLS) + add_ret(response.format(formal_statement=data['output']))
        #print(context)
        #raise
        idx = get_md5(context)
        ...
        back_context = add_ret(back_instruct + '\nLean4 code:\n' + data['output']) + add_ret(back_response.format(informal_statement=NLS))
