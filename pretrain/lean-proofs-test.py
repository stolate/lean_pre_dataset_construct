...
input_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-4/0_ori/lean-proofs-test/lean-proofs-test.jsonl'
output_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-4/1_pretrain/lean-proofs-test/lean-proofs-test.json'
back_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-4/1_pretrain/lean-proofs-test/backtrans_lean-proofs-test.json'
datasource = 'lean-proofs-test'
...
def main():
    ...
    for data in tqdm(indata):
        #print(data)
        NLS = data['prompt'].strip()
        instruction = random.choice(instructions)
        data['expected_response'] = data['expected_response'].replace(':=  by', ':= by')
        if re.search(':=$', data['expected_response']):
            data['expected_response'] = data['expected_response'].strip() + ' by'
        elif re.search('sorry$', data['expected_response'].strip()):
            data['expected_response'] = re.sub('sorry$', '', data['expected_response']).strip()

        if instruction['position'] == 'head':
            if random.random() < 0.5: 
                context = add_ret(instruction['instruction'] + '\nNatural language:\n' + NLS) + add_ret(response.format(target=data['expected_response']))
            else:
                context = add_ret(instruction['instruction'] + NLS) + add_ret(response.format(target=data['expected_response']))
        else:
            if random.random() < 0.5: 
                context = add_ret('Natural language:\n' + NLS + '\n' + instruction['instruction']) + add_ret(response.format(target=data['expected_response']))
            else:
                context = add_ret(NLS + '\n' + instruction['instruction']) + add_ret(response.format(target=data['expected_response']))
        ...
        if back_instruct['position'] == 'head':
            if random.random() < 0.5:
                back_context = add_ret(back_instruct['instruction'] + '\nLean4 code:\n' + data['expected_response']) + add_ret(back_response.format(intarget=NLS))
            else:
                back_context = add_ret(back_instruct['instruction'] + data['expected_response']) + add_ret(back_response.format(intarget=NLS))
        else:
            if random.random() < 0.5:
                back_context = add_ret('Lean4 code:\n' + data['expected_response'] + '\n' + back_instruct['instruction']) + add_ret(back_response.format(intarget=NLS))
            else:
                back_context = add_ret(data['expected_response'] + '\n' + back_instruct['instruction']) + add_ret(back_response.format(intarget=NLS))
