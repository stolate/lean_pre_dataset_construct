...
input_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-4/0_ori/Leanabell-Prover-Traindata-SFT/traindata_sft_without_cot.jsonl'
output_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-4/1_pretrain/Leanabell-Prover-Traindata-SFT/Leanabell-Prover-Traindata-SFT.json'
datasource = 'Leanabell-Prover-Traindata-SFT'
...
def main():
    ...
    org_ins = 'Complete the following Lean 4 code with explanatory comments preceding each line of code:\n\n'

    indata = indent_json2dict(input_p)
    data_lst = []
    for data in tqdm(indata):
        instruction = random.choice(ins_lst)
        data['prompt'] = data['prompt'].replace('```lean4', '').replace('```', '')
        data['output'] = data['output'].replace('```lean4', '').replace('```', '')
        inputtext = data['prompt'].replace(org_ins, '')
        if instruction['position'] == 'head':
            context = add_ret(instruction['instruction'] + '\n' + inputtext) + add_ret(data['output'])
        else:
            context = add_ret(inputtext + '\n' + instruction['instruction']) + add_ret(data['output'])
        idx = get_md5(context)
