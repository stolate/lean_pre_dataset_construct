...
input_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-3/0_ori/MUSTARDSauce/folded_data/filtered_data_all.json'
output_p = '/data3/ftpdata/kmwang5/Lean-dataset/Lean-3/1_pretrain/MUSTARDSauce/MUSTARDSauce-NLS-FLP.json'
datasource = 'MUSTARDSauce'
...
def main():
    ins_path = '/data3/ftpdata/kmwang5/Lean-dataset/instructions/lean3-NLS-FLP.json'
    ...
    for data in tqdm(indata):
        ...
        data['formal_proof_verbose'] = data['formal_proof_verbose'].replace('```lean', '').replace('```', '').strip()
        if instruction['position'] == 'head':
            inputtext = instruction['instruction'] + '\n' + data['informal_statement']
        else:
            inputtext = data['informal_statement'] + '\n' + instruction['instruction']
        context = add_ret(inputtext) + add_ret(data['formal_proof_verbose'])
        idx = get_md5(context)
